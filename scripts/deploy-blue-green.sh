...
GREEN_CLONE_PROJECT="${GREEN_CLONE_PROJECT:-ierp-${IERP_VERSION:-candidate}-clone}"
...
start_clone_candidate() {
  ...
  BACKUP_PATH="$BACKUP_ROOT" \
    docker compose -p "$GREEN_CLONE_PROJECT" \
      -f "$GREEN_COMPOSE_FILE" up -d --force-recreate
  wait_for_health \
...
start_production_candidate() {
  ...
  BACKUP_PATH="$BACKUP_ROOT" \
    docker compose -p "$GREEN_PRODUCTION_PROJECT" \
      -f "$GREEN_COMPOSE_FILE" up -d --force-recreate

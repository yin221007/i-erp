...
import { getProjectBusinessStatus, getProjectBusinessStatusLabel } from '../lib/project-status';
...
const ProjectList: React.FC<ProjectListProps> = ({ 
  ...
  const [selectedManager, setSelectedManager] = useState<string | null>(isRegularUser ? currentUser.nickname : 'all');
  ...
  const managers = useMemo(() => {
    ...
    projects.forEach(p => {
        ...
        entry.count += 1;
        if (getProjectBusinessStatus(p) === 'Active') entry.active += 1;
  ...
  const sortedProjects = useMemo(() => {
      const managerToFilter = selectedManager || (isRegularUser ? currentUser.nickname : 'all');

      let list = projects
          .filter(p => (managerToFilter === 'all' || p.manager === managerToFilter) && (
  ...
  const handleOpenProjectModal = (e: React.MouseEvent | null, project?: Project) => { 
      ...
      if (project) { 
          ...
          setProjectFormData({ 
              name: '', 
              manager: selectedManager && selectedManager !== 'all' ? selectedManager : currentUser.nickname,
              clientName: '', 
              startDate: '', 
              deadline: '', 
              contractNo: '', 
    ...
    <div className="max-w-7xl mx-auto px-1 md:px-0 pb-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
          <div className="w-full md:w-auto">
              {selectedManager && selectedManager !== 'all' && !isRegularUser ? (
                  <>
                    <button onClick={() => { setSelectedManager('all'); setSearchTerm(''); }} className="text-sm text-slate-500 hover:text-primary-600 flex items-center gap-1 mb-2 transition-colors font-bold">
                        <ArrowLeft className="w-4 h-4" /> 返回全部项目
                    </button>
          ...
          <div className="flex items-center space-x-2 w-full md:w-auto overflow-x-auto no-scrollbar">
            ...
            {canCreateProjects && (
                ...
                {sortedProjects.map((project) => {
                    const businessStatus = getProjectBusinessStatus(project);
                    const statusClass = businessStatus === 'Active'
                    ...
                    >
                        <div className="flex justify-between items-start mb-4 md:mb-6">
                            ...
                            <span className={`text-[9px] md:text-[10px] font-black px-2 py-1 md:px-3 md:py-1.5 rounded-full uppercase tracking-widest ${statusClass}`}>{getProjectBusinessStatusLabel(businessStatus)}</span>
                        </div>
